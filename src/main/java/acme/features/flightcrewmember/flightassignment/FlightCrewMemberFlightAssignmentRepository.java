...
public interface FlightCrewMemberFlightAssignmentRepository extends AbstractRepository {
	...
	@Query("select fa from FlightAssignment fa where fa.flightCrewMember.id = :memberId and fa.leg.departure >= moment")
	...
	Collection<Leg> findLegsByFlightAssignment(int id);
	...
	Collection<ActivityLog> findActivityLogsFlightAssignmentId(int flightAssignmentId);
	...
	Collection<FlightAssignment> findFlightAssignmentByLeg(int legId);

	@Query("select case when count(fa) > 0 then true else false end " + "from FlightAssignment fa " + "where fa.id = :flightAssignmentId " + "and fa.leg.departure < :moment")

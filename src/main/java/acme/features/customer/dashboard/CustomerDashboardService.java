...
public class CustomerDashboardService extends AbstractGuiService<Customer, CustomerDashboard> {
	...
	public void load() {
		...
		if (bookings.size() > 0 && bookingRecords.size() > 0) {
			...
			//Last 5 destinations
			List<String> lastFivedestinations = bookings.stream().sorted(Comparator.comparing(Booking::getPurchaseMoment).reversed()).map(b -> b.getFlight().getDestinationCity()).distinct().limit(5).toList();
			dashboard.setLastFiveDestinations(lastFivedestinations);
			System.out.println(lastFivedestinations.size());
			lastFivedestinations.stream().forEach(d -> System.out.println(d));
			//--------------

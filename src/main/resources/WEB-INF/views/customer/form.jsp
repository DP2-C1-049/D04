...
<acme:form readonly="${!draftMode}">
	...
	<jstl:choose>
		...
		<jstl:if test="${draftMode}">
			<acme:submit code="customer.booking.form.button.update" action="/customers/booking/update"/>
			<acme:submit code="customer.booking.form.button.publish" action="/customers/booking/publish"/>
			<jstl:if test="${_command != 'create'}">
				<acme:button code="customer.booking.form.add.passenger" action="/customers/booking-record/create?bookingId=${id}"/>
			</jstl:if>
		</jstl:if>

		</jstl:when>
		<jstl:when test="${_command == 'create'}">
			<acme:submit code="customer.booking.form.button.create" action="/customers/booking/create"/>
		</jstl:when>		
	</jstl:choose>	
	<jstl:if test="${_command != 'create'}">
		<acme:button code="customer.booking.form.show.passengers" action="/customers/passenger/list?bookingId=${id}"/>
	</jstl:if>
